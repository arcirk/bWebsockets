...
int main(int argc, char** argv)
{
    ...
    command_line_parser::cmd_parser parser(argc, argv);
...
//    data.from_email = "myemail@yandex.ru";
    server.server   = "smpts://" + parser.option("-s")  ;
    server.port     = "465"                         ;
    server.user     = parser.option("-from")             ;
    server.password = parser.option("-p")                  ;

    data.filename = "myfile.txt";
    data.filepath = "D:/src/bWebsockets/tests/mail/x64/myfile.txt" ;
    data.from_email = server.user;
    data.to_email.push_back(parser.option("-to"));
