...
//			 СтрШаблон("Клиент отключился от севрвера %1 %2", глВебСокетКлиент.ОбъектКомпоненты.url, ТекущаяДата())));
	...
	Иначе
		...
		НайденнаяФорма = ПолучитьФормуКлиента(ИдентфикаторФормы);
		Если ТипЗнч(НайденнаяФорма) = Тип("ФормаКлиентскогоПриложения") Тогда
//			НайденнаяФорма = ПолучитьФорму("Справочник.ВнешниеПользователи.ФормаВыбора");
//			НайденнаяФорма.УстановитьВыполнениеОбработчиковСобытия();
//			Оповещение = Новый ОписаниеОповещения("ОбработкаОповещения", Э);
		КонецЕсли;
...
Функция ПолучитьФормуКлиента(УникальныйИдентификатор) Экспорт
