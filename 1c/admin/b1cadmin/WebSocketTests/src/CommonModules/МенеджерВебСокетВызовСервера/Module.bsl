...
Функция ПолучитьСтруктуруГлобальныхНастроек() Экспорт
	...
	Настройки.Вставить("ИдентификаторПланаОбмена", "");	
	Возврат Настройки;
КонецФункции
...
Функция СериализоватьОбъект(Объект, ИдентификаторУстройства) Экспорт
	...
	Если Метаданные.Документы.Содержит(МетаданныеОбъекта) Тогда
		...
		СтандартныеРеквизиты.Вставить("number", Объект.Номер);
	КонецЕсли;
	...
	Для Каждого ТабличнаяЧасть Из МетаданныеОбъекта.ТабличныеЧасти Цикл
		...
		Для Каждого ТекСтрока Из Объект[ТабличнаяЧасть.Имя] Цикл
			...
			Для Каждого ТекРеквизит Из МетаданныеОбъекта.ТабличныеЧасти[ТабличнаяЧасть.Имя].Реквизиты Цикл
...
Функция ДесериализоватьОбъект(СтруктураОбъекта) Экспорт
	...
	Если СокрЛП(ТекОбъект[ИмяКлюча]) = "" Тогда
		Если СокрЛП(ТекОбъект.Ссылка[ИмяКлюча]) <> "" Тогда
			ТекОбъект[ИмяКлюча] = ТекОбъект.Ссылка[ИмяКлюча];
		Иначе
			ТекОбъект[ИмяКлюча] = ?(ИмяКлюча = "Номер", ТекОбъект.УстановитьНовыйНомер(), ТекОбъект.УстановитьНовыйКод());
		КонецЕсли;				
	КонецЕсли; 
	...
	мСтрокиДокумента = СтруктураОбъекта.TabularSections;
	мТаблицаСтрок = Новый ТаблицаЗначений();
	мТаблицаСтрок.Колонки.Добавить("ТабличнаяЧасть");
	мТаблицаСтрок.Колонки.Добавить("Штрихкод");
	мТаблицаСтрок.Колонки.Добавить("Количество");
	мТаблицаСтрок.Колонки.Добавить("Цена");	
	Для Каждого ТекСтрока Из мСтрокиДокумента Цикл
		НоваяСтрока = мТаблицаСтрок.Добавить();
		НоваяСтрока.ТабличнаяЧасть = ТекСтрока.first;
	...
	Для Каждого ТЧ Из МетаданныеОбъекта.ТабличныеЧасти Цикл
		...
		мСтроки = мТаблицаСтрок.НайтиСтроки(Новый Структура("ТабличнаяЧасть", ТЧ.Имя));
		Если мСтроки.Количество() > 0 Тогда
			Для Каждого ТекСтрока Из мСтроки Цикл
				НоваяСтрока = ТекОбъект[ТЧ.Имя].Добавить();
