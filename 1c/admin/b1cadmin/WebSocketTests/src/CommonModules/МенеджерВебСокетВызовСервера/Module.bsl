...
Функция ХешТекущегоПользователя() Экспорт
	...
	ИсходнаяСтрока = НРег(ИмяПользователя) + ИдентификаторПользователя;
...
Функция ПрочитатьОтветСервера(ОтветСервера) Экспорт
	...
	Исключение
		СтруктураДанных.Вставить("Ошибка,ОписаниеОшибки", "МенеджерВебСокетВызовСервера::ПрочитатьОтветСервера", ОписаниеОшибки());
	КонецПопытки;
