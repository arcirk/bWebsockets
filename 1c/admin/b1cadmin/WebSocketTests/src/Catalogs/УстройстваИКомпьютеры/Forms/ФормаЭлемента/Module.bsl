...
Процедура УстановитьПараметрыЗаписиНаСервере(СтруктураЗаписи)
	...
	Если ЗначениеЗаполнено(мОбъект.ТипУстройства) Тогда
		МенеджерВебСокетВызовСервера.ОтветСервераУстановитьЗначение(СтруктураЗаписи, "deviceType", СокрЛП(мОбъект.ТипУстройства));
	КонецЕсли;
	...
	МенеджерВебСокетВызовСервера.ОтветСервераУстановитьЗначение(СтруктураЗаписи, "price_type", XMLСтрока(мОбъект.ТипЦен));
