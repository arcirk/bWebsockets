...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьНадписьПустогоПароляИБ();
	УстановитьНадписьПустогоПароляНаСервереWebSocket();
...
//	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекущийОбъект.Ссылка) Тогда		
		...
		ХранилищеОбщихНастроек.Сохранить(КлючОбъекта, КлючНастроек, ЛокальныеНастройки, , ТекущийОбъект.Код);
	КонецЕсли;
...
Процедура УстановитьПараметрыЗаписиНаСервере(СтруктураЗаписи)
	...
	МенеджерВебСокетВызовСервера.ОтветСервераУстановитьЗначение(СтруктураЗаписи, "is_group", 0);
КонецПроцедуры

&НаСервере
Процедура УстановитьНадписьПустогоПароляНаСервереWebSocket()
...
&НаСервере
Процедура УстановитьНадписьПустогоПароляИБ()
	ТекстНадписи = "Пустой пароль";
	Если ЗначениеЗаполнено(Объект.ИдентификаторПользователяИБ) Тогда
