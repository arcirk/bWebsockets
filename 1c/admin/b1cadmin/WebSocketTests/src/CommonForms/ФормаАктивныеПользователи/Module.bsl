...
Асинх Процедура ПривязатьКУстройству(Команда)
	...
	Если РезультатВыбора <> Неопределено Тогда
		Если РезультатВыбора.Значение = КодВозвратаДиалога.Нет Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;

	Оповещение = Новый ОписаниеОповещения("ПриВыбореУстройства", ЭтотОбъект, Новый Структура("ТекущаяСтроока", ТекущаяСтрока));
