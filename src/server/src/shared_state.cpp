...
//    else
        session_ssl_.insert(std::pair<boost::uuids::uuid, websocket_session_ssl*>(session->get_uuid(), session));
}
...
//                 arcirk::uuids::uuid_to_string(session->get_uuid()) + "\"}", "client_leave", true, session->notify_apps());
    ...
    auto iter = session_ssl_.find(session->get_uuid());
    if (iter != session_ssl_.end() ){
        session_ssl_.erase(session->get_uuid());
    }
...
//    }else{
        ...
        {
            std::lock_guard<std::mutex> lock(mutex_);
            v.reserve(session_ssl_.size());
            for(auto p : session_ssl_){
                v.emplace_back(p.second->weak_from_this());
            }
